...
class EditApartmentPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Title
        label = tk.Label(self, text="Edit Apartment Details", font=("Arial", 18, "bold"))
        label.pack(pady=20)
        ...
        # 1. Location (ReadOnly or Dropdown)
        ...
        # 4. Status (Crucial for occupancy tracking)
        ...
        update_btn = tk.Button(btn_frame, text="Update Apartment", bg="#f39c12", fg="white", 
                               width=15, command=self.update_data)
        update_btn.pack(side="left", padx=10)

        delete_btn = tk.Button(btn_frame, text="Delete Record", bg="#e74c3c", fg="white", 
                               width=15, command=self.confirm_delete)
        delete_btn.pack(side="left", padx=10)

        back_btn = tk.Button(btn_frame, text="Cancel", command=lambda: self.controller.show_frame("ApartmentListPage"))
        back_btn.pack(side="left", padx=10)

    def update_data(self):
        # Validation logic (similar to Add page)
        print("Sending update to MySQL...")
        messagebox.showinfo("Success", "Apartment details updated.")

    def confirm_delete(self):
        # Safety check before deleting
        answer = messagebox.askyesno("Confirm Delete", "Are you sure you want to remove this apartment? This cannot be undone.")
        if answer:
