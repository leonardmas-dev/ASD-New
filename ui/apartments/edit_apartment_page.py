...
class EditApartmentPage(tk.Frame):
    # Added parameters to catch data from the list page
    def __init__(self, parent, controller, apt_id=None, location="", apt_type="", rent="", status="", **kwargs):
        ...
        self.location_cb.set(location) # Fill location
        ...
        self.type_cb.set(apt_type) # Fill type
        ...
        self.status_cb.set(status) # Fill status
        ...
        tk.Button(btn_frame, text="Update Apartment", bg="#f39c12", fg="white", width=15, 
                  command=self.update_data).pack(side="left", padx=10)
        
        tk.Button(btn_frame, text="Delete Record", bg="#e74c3c", fg="white", width=15, 
                  command=self.confirm_delete).pack(side="left", padx=10)
        
        tk.Button(btn_frame, text="Cancel", 
                  command=lambda: self.controller.load_page(ApartmentListPage)).pack(side="left", padx=10)
