...
class TenantPaymentGraphsPage(tk.Frame):
    ...
    def __init__(self, parent, main_window):
        ...
        tk.Button(btn_frame, text="Payment History",
                  command=self.show_payment_history_graph).grid(row=0, column=0, padx=5)

        tk.Button(btn_frame, text="Me vs Neighbours",
                  command=self.show_neighbours_graph).grid(row=0, column=1, padx=5)

        tk.Button(btn_frame, text="Late Payments per Property",
                  command=self.show_late_payments_graph).grid(row=0, column=2, padx=5)
    ...
    def show_payment_history_graph(self):
        ...
        if not payments:
            messagebox.showinfo("No Data", "No payments found to display.")
            return
    ...
    def show_neighbours_graph(self):
        ...
        if not summary:
            messagebox.showinfo("No Data", "No neighbour payment data available.")
            return
